...
for key in nsis_path:
    ...
    if os.path.exists(pld_exe_path):
        fw_pgm = subprocess.Popen(
            r'netsh advfirewall firewall delete rule name="{}" program="{}"'.format(key, pld_exe_path), shell=True,
            stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        fw_pgm_output, errors = fw_pgm.communicate()
        tsk = subprocess.Popen(r'schtasks /delete /tn {}_ST /f'.format(key), shell=True, stdout=subprocess.PIPE,
